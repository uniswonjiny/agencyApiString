...
public class KSPaySeed {

	private static final int SS0[] = { 0x2989a1a8, 0x5858184, 0x16c6d2d4, 0x13c3d3d0,
	...
	private static final int SS1[] = { 0x38380830, 0xe828c8e0, 0x2c2d0d21, 0xa42686a2,
	...
	private static final int SS2[] = { 0xa1a82989, 0x81840585, 0xd2d416c6, 0xd3d013c3,
	...
	private static final int SS3[] = { 0x8303838, 0xc8e0e828, 0xd212c2d, 0x86a2a426,
	...
	private static final int KC[] = { 0x9e3779b9, 0x3c6ef373, 0x78dde6e6, 0xf1bbcdcc,
	...
	{
		SeedEncRoundKey(sRoundKey, KSPayMsg.s2b(pbUserKeyStr));
	}
	...
	byte[] addPadding(byte[] sbuf,int pMode,int bSize)
	{
		...
		if (plen == bSize && pMode != PKCS5_PADDING) plen  = 0;

		byte[] rbuf = new byte[sbuf.length+plen];
		System.arraycopy(sbuf,0,rbuf,0,sbuf.length);

		if (NULL_PADDING == pMode || 0 == plen) return rbuf ;

		if		(SPACE_PADDING == pMode) pchar = (byte)0x20;
		else if	(PKCS5_PADDING == pMode) pchar = (byte)plen;
		else	throw new IllegalArgumentException ("Padding("+pMode+") - Not Implemented!!");
	...
	byte[] removePadding(byte[] sbuf,int pMode,int bSize)
	{
		...
		if		(NULL_PADDING  == pMode) pchar = (byte)0x00;
		else if	(SPACE_PADDING == pMode) pchar = (byte)0x20;
		else if	(PKCS5_PADDING == pMode) pchar = sbuf[sbuf.length-1];
		else	throw new IllegalArgumentException ("Padding("+pMode+") - Not Implemented!!");
...
//System.out.println("cbc_encrypt 3."+j+" "+KSPaySeed.hex_encode(xOutput)+" = "+KSPaySeed.hex_encode(xInput)+" ^ "+KSPaySeed.hex_encode(inData)+" ");
...
//System.out.println("cbc_encrypt 4.패딩후 ("+KSPaySeed.hex_encode(xOutput)+")");
	...
	public static String hex_encode(byte sbuf[])
	{
	    ...
	    return KSPayMsg.b2s(rbuf);
	}

	public static byte[] hex_decode(String sStr)
	{
	 	if (sStr.length() != sStr.length() / 2 * 2) return null;
